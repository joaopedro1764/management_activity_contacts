    ...
    <>
      ...
      >
        ...
        {isOpenSidebar && (
          <button
            onClick={
              isMobile ? handleOpenAndCloseSidebar : handleCollapseSidebar
            }
            className={`absolute p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-100 rounded-full transition-all duration-200 z-10 ${
              isMobile
                ? "top-4 right-4"
                : isCollapsed
        ...
        >
          ...
          <ul className="flex flex-col gap-3 flex-1">
            {menuItems
              .filter((item) => {
                if (item.link === "/dashboard" && user?.tipo !== "admin")
                  return false;
                return true;
              })
              .map((item) => {
              ...
              <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-200 backdrop-blur-sm mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="overflow-hidden">
                    <div className="text-blue-800 font-medium text-sm truncate">
                      {user?.nome}
                    </div>
                    <div className="text-blue-600 text-xs truncate">
                      {user?.tipo === "admin" ? "Admnistrador" : "Vendedor"}
                    </div>
                  </div>
                </div>
              </div>
